...
oci8_make_pval(pval *value,oci8_statement *statement,oci8_out_column *column, char *func, int mode)
{
	...
	if (column->indicator || column->retcode)
		if ((column->indicator != -1) && (column->retcode != 1405))
			oci8_debug("oci8_make_pval: %16s,retlen = %4d,retlen4 = %d,storage_size4 = %4d,indicator %4d, retcode = %4d",
...
oci8_fetch(oci8_statement *statement, ub4 nrows, char *func)
{
    ...
    while (statement->error == OCI_NEED_DATA) {
        for (i = 0; i < statement->ncolumns; i++) {
            ...
            if (column->piecewise)  {
                if (! column->data) {
                    column->data = (text *) emalloc(OCI8_PIECE_SIZE);
                } else {
                    column->data = erealloc(column->data,column->retlen4 + OCI8_PIECE_SIZE);
                }
 
                column->cb_retlen = OCI8_PIECE_SIZE;
...
/* {{{ proto int OCIDefineByName(int stmt, string name, mixed &var [, int type])
   Define a PHP variable to an Oracle column by name. If you want to define a LOB/CLOB etc make sure you allocate it via OCINewDescriptor BEFORE defining. */
...
/* {{{ proto int OCIBindByName(int stmt, string name, mixed &var, int maxlength [, int type])
   Bind a PHP variable to an Oracle placeholder by name. If you want to bind a LOB/CLOB etc make sure you allocate it via OCINewDescriptor BEFORE binding. */
...
PHP_FUNCTION(oci8_savedesc)
{
	...
	if ((id = getThis()) != 0) {
	    ...
	    if (getParameters(ht, 2, &arg, &oarg) == SUCCESS) {
			...
			oci8_debug("OCIsavedesc: curloblen=%d",curloblen);
		...
		oci8_debug("OCIsavedesc: size=%d offset=%d",loblen,offset);
