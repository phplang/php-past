...
/* $Id: rpc.c,v 1.2 1999/04/22 13:19:50 ssb Exp $ */
...
#ifdef THREAD_SAFE
# define RPC_GLOBAL(a) rpc_globals->a
# define RPC_TLS_VARS \
	rpc_global_struct *rpc_globals = TlsGetValue(RPCTls); 

void *rpc_mutex;
DWORD RPCTls;
static int numthreads=0;

typedef struct rpc_global_struct {
...
/* NOTE: You should have an rpc.def file where you export DllMain */
BOOL WINAPI DllMain(HANDLE hModule, DWORD  ul_reason_for_call, 
					LPVOID lpReserved)
{
    switch (ul_reason_for_call) {
		case DLL_PROCESS_ATTACH:
			if ((RPCTls = TlsAlloc()) == 0xFFFFFFFF){
				return 0;
			}
			break;    
