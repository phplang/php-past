...
static void _free_result(UODBC_RESULT *res)
{
	...
	if (res){
		...
		if (res->stmt){
			...
			if(_php3_hash_find(UODBC_GLOBAL(PHP3_UODBC_MODULE).conns,
				(void*)&(res->conn_ptr), sizeof(void*), (void **)&db_conn) == SUCCESS){
#if HAVE_SOLID
...
*/
...
#if HAVE_SOLID
	SQLTransact(UODBC_GLOBAL(PHP3_UODBC_MODULE).henv, conn->hdbc,
				(UWORD)SQL_COMMIT);
#endif
	...
	_php3_hash_del(UODBC_GLOBAL(PHP3_UODBC_MODULE).conns, (void*)&conn, sizeof(void*));
}
...
int PHP3_MINIT_UODBC(INIT_FUNC_ARGS) 
{
	...
	UODBC_GLOBAL(PHP3_UODBC_MODULE).conns = malloc(sizeof(HashTable));
	_php3_hash_init(UODBC_GLOBAL(PHP3_UODBC_MODULE).conns, 13, NULL, NULL, 1);
...
{ 
	...
	char *filename;
   	SWORD sqltype, scale, nullable, ctype;
	UDWORD precision;
   	UODBC_RESULT *result=NULL;
	...
	if (result->numparams > 0){
		...
		for (i = 1; i <= result->numparams; i++) {
			if (_php3_hash_get_current_data(arr.value.ht, (void **) &tmp) 
						== FAILURE) {
...
void PHP3_UODBC_DO_CONNECT(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	*/
	...
	if ((persistent || (_php3_hash_find(list, hashed_details, hashed_len + 1,
	 ...
	 _php3_hash_add(UODBC_GLOBAL(PHP3_UODBC_MODULE).conns,
					(void*)&db_conn,
					sizeof(void*),
					(void*)&db_conn,
					sizeof(void*),
