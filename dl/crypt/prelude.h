...
/*  ----------------------------------------------------------------<Prolog>-
    ...
    Version:    1.81
    <TABLE>
    1.81_PH     Defined TIMEZONE as 0 for FreeBSD
    1.80_PH     Released with SFL 1.80
    </TABLE>
...
#       include <direct.h>              /*    so is __MSDOS__                */
#   endif
 ...
 */
/* not used for our crypt functions in php*/
#if (0)
void sys_assert (const char *file, unsigned line);
#   undef  ASSERT
#   define ASSERT(f)    \
        if (f)          \
            ;           \
...
#elif (defined (__UTYPE_SCO) || defined (__UTYPE_UNIXWARE))
    extern char **environ;              /*  Not defined in include files     */
