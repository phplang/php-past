...
char *php_re_compile_pattern(regex, size, bufp)
char *regex;
int size;
regexp_t bufp;
{
...
void php_re_compile_fastmap(bufp)
regexp_t bufp;
{
...
#define NEXTCHAR(var)							\
	...
	if (pos <= size1) {
		...
		if(string2) part_2_end = string2 + mstop - size1;
		else part_2_end = NULL;
/*		part_2_end = string2 + mstop - size1;  / * non-ANSI pointer arithmetic here */
	} else {
...
continue_matching:
	for (;;) {
		switch (*code++) {
		case Cend:
			if (partend != part_2_end) match_end = text - string1;
			else match_end = text - string2 + size1;
			if (regs) {
				regs->start[0] = pos;
		...
		case Ceol:
			if ((string2 && text == string2+size2) || 
			   (text==string1+size1?(size2==0||(string2 && *string2=='\n')) : *text == '\n'))
				break;
			goto fail;
		...
		case Cmatch_memory:
			reg = *code++;
			if (regend_text[reg] == NULL) goto fail;  /* or should we just match nothing? */
			regtext = regstart_text[reg];
			regtextend = regend_text[reg];
			if (regstart_partend[reg] == regend_partend[reg]) regpartend = regtextend;
			else regpartend = string1 + size1;
	  
			for(;regtext != regtextend;) {
				NEXTCHAR(ch);
				if (regtext == regpartend) regtext = string2;
				regch = (unsigned char)*regtext++;
				if (translate) regch = (unsigned char)translate[regch];
				if (regch != ch) goto fail;
			}
			break;
	...
	case Cstar_jump:
		...
		a = (int)(short)a;
		{
			...
			if(!re_do_compile_fastmap(bufp->buffer, bufp->used,p2-bufp->buffer,&can_be_null,map))
	...
	loop_p1:
			...
			case Cexact:
				ch = (unsigned char)*p1++;
				if(map[ch]) goto make_normal_jump;
				break;
	...
	case Cfailure_jump:
		if (failure_sp == failure_stack_end) {
			if (failure_stack_start != initial_failure_stack) goto error;
	...
	case Cwordbeg:
		if (text == string2 + size2) goto fail;
		if (size2 == 0 && text == string1 + size1) goto fail;
		if (SYNTAX(text == string1 + size1 ? *string1 : *text) != Sword) goto fail;
		if (text == string1) break;
		if (SYNTAX(text[-1]) != Sword) break;
		goto fail;

	case Cwordend:
		if (text == string1) goto fail;
		if (SYNTAX(text[-1]) != Sword) goto fail;
		if (text == string2 + size2) break;
		if (size2 == 0 && text == string1 + size1) break;
	...
	case Csyntaxspec:
		NEXTCHAR(ch);
		if ((unsigned char)SYNTAX(ch) != (unsigned char)*code++) goto fail;
		break;

	case Cnotsyntaxspec:
		NEXTCHAR(ch);
		if ((unsigned char)SYNTAX(ch) != (unsigned char)*code++) break;
		goto fail;
...
int php_re_match(bufp, string, size, pos, regs)
regexp_t bufp;
char *string;
int size, pos;
...
int php_re_search(bufp, string, size, startpos, range, regs)
regexp_t bufp;
char *string;
...
char *s;
{
  ...
  return php_re_compile_pattern(s, strlen(s), &re_comp_buf);
}
...
char *s;
{
  ...
  return php_re_search(&re_comp_buf, s, len, 0, len, (regexp_registers_t)NULL) >= 0;
}
  ...
  while (1)
    {
      printf("Enter regexp:\n");
      gets(buf);
      cp=php_re_compile_pattern(buf, strlen(buf), &exp);
      if (cp)
	{
	  printf("Error: %s\n", cp);
	  continue;
      ...
      while (fgets(buf, sizeof(buf), stdin))
	{
	  if (buf[0] == '\n')
	    break;
	  a = php_re_search(&exp, buf, strlen(buf), 0, strlen(buf), &regs);
